...
void adjustHeap(DynamicArray* heap, int last, int position, compareFunction compare)
{
    ...
    if(rightIdx < last)
    {
        if(compare(dyGet(heap, leftidx), dyGet(heap, rightIdx)) == -1)
        {
            lowChild = leftidx;
        }
        else
        ...
        if (compare(dyGet(heap, lowChild), dyGet(heap, position)) == -1)
        {
            dySwap(heap, position, lowChild);
            adjustHeap(heap, last, lowChild, compare);
        }
    }
...
void dyHeapAdd(DynamicArray* heap, TYPE value, compareFunction compare)
{
    ...
    void* parent_value = heap->data[parent];
