...
struct Node *_addNode(struct Node *cur, TYPE val)
{
   ...
   if( cur == NULL)             /*Adds node if bottom of tree. Otherwise branches and calls recursively*/
   {
...
{
    assert((val!=NULL)&&(tree != NULL));
    
    struct Node* cur = tree->root;
    
    while (cur != NULL)
    {
        if (compare(val, cur->val) == 0)       //if values are the same return true
        {
            return 1;
        }
        else if (compare(val, cur->val) == -1) //else keep searching
        {
            cur = cur->left;
        }
        else
